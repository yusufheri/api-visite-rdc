...
const q2 = `SELECT  s.id, s.name, s.description, s.latitude, s.longitude,s.altitude, CONCAT("${url}", s.illustration) AS illustration, rs.restaurant_id as restaurant, s.phone,s.email, s.website FROM site AS s, restaurant_site AS rs WHERE s.id=rs.site_id`,
